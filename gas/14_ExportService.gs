function exportKegiatan_(body, payload) {
  ...
  const exportFolderId =
    PropertiesService.getScriptProperties().getProperty(CONFIG.PROPERTIES.EXPORT_FOLDER_ID) ||
    PropertiesService.getScriptProperties().getProperty(CONFIG.PROPERTIES.DRIVE_FOLDER_ID);
  if (!exportFolderId) {
    throw new Error("Property EXPORT_FOLDER_ID atau DRIVE_FOLDER_ID belum diatur.");
  }
  ...
  const copiedFile = DriveApp.getFileById(templateId).makeCopy(copyName, DriveApp.getFolderById(exportFolderId));
  ...
  const exportFile = DriveApp.getFileById(copiedFile.getId());
  const mimeType = format === "pdf" ? MimeType.PDF : MimeType.MICROSOFT_WORD;
  const extension = format === "pdf" ? ".pdf" : ".docx";
  const blob = exportFile.getAs(mimeType).setName(copyName + extension);
  const generatedFile = DriveApp.getFolderById(exportFolderId).createFile(blob);
  generatedFile.setSharing(DriveApp.Access.ANYONE_WITH_LINK, DriveApp.Permission.VIEW);

  logAction(user.user_id, "export_kegiatan_" + format, kegiatanId);
  return {
    fileUrl: generatedFile.getUrl(),
    docUrl: exportFile.getUrl()
  };
}
